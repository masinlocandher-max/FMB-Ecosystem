...
if (!html.includes(storyHref)) {
  ...
  if (!html.includes(wireNeedle)) throw new Error('FMB News tax-records feature: news wire not found');
  html = html.replace(
    wireNeedle,
  ...
  if (rundownInsertPoint < 6) throw new Error('FMB News tax-records feature: rundown header not found');
  ...
  if (!html.includes(listNeedle)) throw new Error('FMB News tax-records feature: structured story list not found');
  html = html.replace(
    listNeedle,
