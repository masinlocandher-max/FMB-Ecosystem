...
const attempt = async (name, action) => { try { await action(); } catch (error) { record(`${name}: fatal`, false, error?.stack || String(error)); } };
...
try {
  await attempt('desktop home', async () => {
    ...
    const drawer = page.locator('[data-fmbn-drawer]');
    record('desktop: no permanent sidebar', await drawer.getAttribute('aria-hidden') === 'true' && !(await page.locator('body').evaluate((node) => node.classList.contains('fmbn-drawer-open'))), 'drawer remains closed');
  ...
  await attempt('mobile home and drawer', async () => {
  ...
  await attempt('archives and article', async () => {
    const context = await browser.newContext({ viewport: { width: 1440, height: 1000 } });
    const page = await context.newPage();
    ...
    await snap(page, 'desktop-archives.png');
    ...
    if (firstRoute) {
      ...
      record('article: body preserved', (await articleMain.innerText()).length > 400, `length=${(await articleMain.innerText()).length}`);
