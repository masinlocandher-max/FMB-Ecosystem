...
function watchConsole(page,label){
  page.on('console',message=>{
    ...
    results.console.push({page:label,type:message.type(),text});
  });
  ...
  page.on('requestfailed',request=>{
    ...
    results.console.push({page:label,type:'requestfailed',text:`${request.method()} ${url.pathname}: ${request.failure()?.errorText||'failed'}`});
  });
}
...
const pageState=()=>{
  ...
  return {
    ...
    desktopSubmitVisible:visible(desktopSubmit),
    desktopSubmitHref:desktopSubmit?.getAttribute('href')||'',
    mobileSubmitVisible:visible(mobileSubmit),
    mobileSubmitHref:mobileSubmit?.getAttribute('href')||'',
...
try{
  ...
  requireCheck('desktop has no horizontal overflow',landing.overflow<=1,{overflow:landing.overflow,offenders:landing.overflowOffenders});

  const totalCards=await desktop.locator('[data-fnc-card]').count();
  await desktop.locator('[data-fnc-search]').fill('Katrina');
  ...
  await desktop.locator('[data-fnc-filter="world"]').click();
  await desktop.waitForTimeout(120);
  ...
  await footerLogo.screenshot({path:path.join(evidenceDir,'fmbnews-approved-desktop-footer.png')});
  results.screenshots.push('fmbnews-approved-desktop-footer.png');
  ...
  requireCheck('mobile has no horizontal overflow',mobileClosed.overflow<=1,{overflow:mobileClosed.overflow,offenders:mobileClosed.overflowOffenders});

  await mobile.locator('[data-fnc-menu]').click();
  ...
  await mobile.locator('[data-fmb-news-logo-dark]').scrollIntoViewIfNeeded();
  await mobile.waitForTimeout(150);
  ...
  await mobile.locator('[data-fmb-news-logo-dark]').screenshot({path:path.join(evidenceDir,'fmbnews-approved-mobile-footer.png')});
  results.screenshots.push('fmbnews-approved-mobile-footer.png');
  ...
  await article.goto(`${baseUrl}${articlePath}`,{waitUntil:'domcontentloaded'});
  ...
  const articleState=await article.evaluate(()=>{
    ...
    const viewportWidth=document.documentElement.clientWidth;
    const overflowOffenders=[...document.querySelectorAll('body *')]
      .map(element=>{const rect=element.getBoundingClientRect();return{tag:element.tagName.toLowerCase(),className:typeof element.className==='string'?element.className:'',left:Math.round(rect.left),right:Math.round(rect.right),width:Math.round(rect.width)}})
      .filter(item=>item.right>viewportWidth+1||item.left<-1)
      .slice(0,12);
    return {
      clean:document.body.classList.contains('fmb-news-clean'),
      lightSrc:light?.getAttribute('src')||'',
      darkSrc:dark?.getAttribute('src')||'',
      lightLoaded:Boolean(light?.complete&&light.naturalWidth>0),
      darkLoaded:Boolean(dark?.complete&&dark.naturalWidth>0),
      imageLoaded:Boolean(image?.complete&&image.naturalWidth>0&&image.naturalHeight>0),
      submissionLinks:document.querySelectorAll('[data-fmb-story-submission]').length,
      sharePanels:document.querySelectorAll('[data-fmb-share-ready]').length,
      shareIcons:document.querySelectorAll('[data-fmb-share-ready] .fnc-share-icon').length,
      socialLinks:document.querySelectorAll('[data-fmb-share-ready] a').length,
      nativeButtons:document.querySelectorAll('[data-fnc-native-share]').length,
      overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,
      overflowOffenders,
    };
  });
  ...
  requireCheck('current report has no horizontal overflow',articleState.overflow<=1,{overflow:articleState.overflow,offenders:articleState.overflowOffenders});

  const sharePanel=article.locator('[data-fmb-share-ready]');
  await sharePanel.scrollIntoViewIfNeeded();
  await article.waitForTimeout(120);
  await sharePanel.screenshot({path:path.join(evidenceDir,'fmbnews-approved-mobile-article-share.png')});
  results.screenshots.push('fmbnews-approved-mobile-article-share.png');
  await article.locator('[data-fnc-native-share]').click();
