...
function resolveAsset(file,siteRoot,html,src){
  const clean=decodeURIComponent(src.split(/[?#]/)[0]);
  if(!clean||/^(?:https?:|data:|blob:|mailto:|tel:|javascript:)/i.test(clean))return;
  if(clean.startsWith('/'))return path.join(siteRoot,clean.replace(/^\//,''));
  const baseHref=html.match(/<base\b[^>]*href=["']([^"']+)["']/i)?.[1];
  if(baseHref){
    if(/^https?:/i.test(baseHref))return;
    const baseDirectory=baseHref.startsWith('/')?path.join(siteRoot,baseHref.replace(/^\//,'')):path.resolve(path.dirname(file),baseHref);
...
for(const file of (await walk(root)).filter(file=>file.endsWith('.html'))){
  ...
  for(const tag of tags){
    ...
    const src=tag.match(/\ssrc=["']([^"']+)["']/i)?.[1];if(!src)continue;
    const target=resolveAsset(file,siteRoot,html,src);if(!target)continue;
    try{
      if(!(await stat(target)).isFile())continue;
      const size=await dimensions(target);if(!size||!size.width||!size.height)continue;
      let replacement=tag;
      if(!/\swidth=/i.test(replacement))replacement=replacement.replace(/<img/i,`<img width="${size.width}"`);
      if(!/\sheight=/i.test(replacement))replacement=replacement.replace(/<img/i,`<img height="${size.height}"`);
      html=html.replace(tag,replacement);changed=true;updatedImages++;
    }catch{}
  }
