...
const cssDistPath = path.join(distRoot, 'assets', 'css', 'fmbnews-independent-v7.css');
const version = '20260803-independent-v7';
...
function publicationHeader(about = false) {
  return `<header class="nc-site-header fn7-site-header" id="top">
    <div class="fn7-nav-shell">
      <a class="fn7-wordmark" href="/fmbnews/" aria-label="FMB News home"><span>FMB</span><strong>NEWS</strong></a>
      <nav class="nc-site-links fn7-site-links" id="newsNav" aria-label="FMB News navigation">
        <a href="/fmbnews/#rundown"${about ? '' : ' aria-current="page"'}>Latest</a>
        <a href="/fmbnews/about/"${about ? ' aria-current="page"' : ''}>About</a>
      </nav>
      <a class="fn7-home-button" href="/fmbandco/">FMB&amp;CO. Home</a>
      <button class="nc-menu-toggle fn7-menu-toggle" type="button" data-news-menu aria-label="Open news menu" aria-expanded="false" aria-controls="newsNav"><span></span><span></span></button>
      ${auditOnly()}
    </div>
  </header>`;
}
...
function addIndependentStylesheet(html) {
  const link = `<link rel="stylesheet" href="/assets/css/fmbnews-independent-v7.css?v=${version}" data-fmb-news-independent-v7>`;
  return html
    .replace(/<link\b[^>]*data-fmb-news-independent-v7[^>]*>\s*/gi, '')
    .replace(/<\/head>/i, `${link}</head>`);
}
...
function hiddenUnifiedShell() {
  return `<header class="fmb-shell-header" hidden><a href="/"><img src="/assets/images/fmbandco/fmbandco-primary-reversed.png" width="1414" height="405" alt="FMB&amp;CO."></a></header>`;
}

function hiddenUnifiedFooter() {
  return `<footer class="fmb-shell-footer" hidden><img src="/assets/images/fmbandco/fmbandco-primary-reversed.png" width="1414" height="405" alt="FMB&amp;CO."></footer>`;
}
...
for (const filePath of targets) {
  ...
  html = addIndependentStylesheet(html);
  ...
  if (!html.includes('data-fmb-news-independent-v7')) throw new Error(`Independent FMB News stylesheet missing: ${filePath}`);
...
for (const [root, canonical] of [[fmbNewsRoot, canonicalAbout], [newsRoot, canonicalAbout]]) {
  ...
  await writeFile(path.join(aboutDirectory, 'index.html'), aboutPage(canonical), 'utf8');
}
