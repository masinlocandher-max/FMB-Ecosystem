...
const runtimeVirtualPrefixes = ['/_vercel/'];
const failures = [];
...
async function listFiles(directory, excludedDirectories = new Set(), relativeDirectory = '') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) files.push(...await listFiles(absolutePath, excludedDirectories, relativePath));
    else if (entry.isFile()) files.push(relativePath);
  }
  return files;
}

function decodeHtmlAttribute(value) {return value.replaceAll('&amp;', '&').replaceAll('&quot;', '"').replaceAll('&#39;', "'").replace(/&#(\d+);/g, (_, number) => String.fromCodePoint(Number(number))).trim();}
function isDynamicReference(reference) {return reference.includes('{{')||reference.includes('}}')||reference.includes('${')||reference.startsWith('#!');}
function documentPath(relativeFile) {const normalized=relativeFile.split(path.sep).join('/');return normalized.endsWith('/index.html')?`/${normalized.slice(0,-'index.html'.length)}`:`/${normalized}`;}
function normalizeHostedPath(host, pathname) {if(host==='data.francinemariebautista.com')return'/admin.html';if(host==='app.francinemariebautista.com'&&(pathname==='/'||pathname===''))return'/app/index.html';if(host==='yoni.francinemariebautista.com'){if(pathname==='/'||pathname==='')return'/app/index.html';if(!pathname.startsWith('/app/')&&!pathname.startsWith('/assets/'))return'/app/index.html';}return pathname;}
async function resolveFile(siteRoot,pathname){let decodedPath;try{decodedPath=decodeURIComponent(pathname)}catch{return null}const relativePath=decodedPath.replace(/^\/+/,''),directPath=path.resolve(siteRoot,relativePath);if(directPath!==siteRoot&&!directPath.startsWith(`${siteRoot}${path.sep}`))return null;const candidates=[directPath,path.join(directPath,'index.html')];if(!path.extname(directPath))candidates.push(`${directPath}.html`);for(const candidate of candidates){try{if((await stat(candidate)).isFile())return candidate}catch{}}return null;}
function hasFragment(html,fragment){let decodedFragment;try{decodedFragment=decodeURIComponent(fragment)}catch{return false}const escaped=decodedFragment.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');return new RegExp(`\\b(?:id|name)\\s*=\\s*["']${escaped}["']`,'i').test(html);}
...
for(const site of sites){const files=await listFiles(site.root,site.excludeDirectories);for(const relativeFile of files.filter(file=>file.endsWith('.html')))await checkHtml(site,relativeFile);for(const relativeFile of files.filter(file=>file.endsWith('.css')))await checkCss(site,relativeFile)}
