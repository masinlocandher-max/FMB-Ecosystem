...
for(const file of htmlFiles){
  ...
  if(name==='index.html'||/^(aboutfmb|withlovefmb|news|fmbandco)\//.test(name)){if(!/<meta\s+name=["']description["']/i.test(html))add('error','missing SEO description');if(!/<link\s+rel=["']canonical["']/i.test(html))add('error','missing canonical URL');}
  pages.push({file:name,bytes:Buffer.byteLength(html),issues});
}

const requiredRoutes=['index.html','aboutfmb/index.html','withlovefmb/index.html','communityengagements/index.html','news/index.html','fmbandco/index.html','app/index.html','app/install/index.html'];
