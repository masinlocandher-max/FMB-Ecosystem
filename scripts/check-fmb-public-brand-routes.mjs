...
const readingRoutes=['coming-out-respect.html','dress-with-intention.html','men-can-cry.html','reading.html','skin-care-makeup.html','womens-health.html','herra.html'];
...
for(const file of await walk(root)){
  ...
  for(const retired of ['/assets/images/home/fmb-home-logo.webp','/assets/images/fmb-official-2026/fmb-master-square.webp','/assets/images/fmb-official-2026/fmb-news-official.webp','/assets/images/fmb-official-2026/fmb-music-official.webp'])if(html.includes(retired))fail(`${name} still exposes retired identity ${retired}`);
  if(name.startsWith('news/')){requireMarker(html,name,exactChannels.news);newsPages+=1;}
...
for(const [route,marker] of Object.entries(representativeRoutes)){const html=await readFile(path.join(root,route),'utf8');requireMarker(html,route,marker);}
console.log(`FMB public-route brand audit verified ${publicPages} public HTML pages, including ${newsPages} News routes and ${readingRoutes.length} reading routes, with exact masters archived and browser-safe official channel delivery.`);
