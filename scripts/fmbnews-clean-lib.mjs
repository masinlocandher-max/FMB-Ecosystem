...
export async function articleRecords(newsRoot){const out=[];for(const file of await walk(newsRoot)){if(path.basename(file)!=='index.html')continue;const slug=path.basename(path.dirname(file));if(slug==='news'||slug==='about'||excluded.has(slug))continue;const html=await readFile(file,'utf8');if(/http-equiv=(['"])refresh\1/i.test(html)||/<meta\b[^>]*(?:name|property)=(['"])robots\1[^>]*content=(['"])[^'"]*noindex/i.test(html))continue;if(!/<h1\b/i.test(html))continue;const r=record(html,`/news/${slug}/`);if(validRecord(r))out.push(r)}return out}
