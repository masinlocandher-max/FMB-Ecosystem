...
export function merge(a,b){const map=new Map();for(const r of [...a,...b]){if(!r)continue;const current=map.get(r.route);if(!current){map.set(r.route,r);continue}if(!Number.isFinite(Date.parse(current.publishedAt||''))&&Number.isFinite(Date.parse(r.publishedAt||'')))map.set(r.route,{...current,publishedAt:r.publishedAt,published:r.published})}return[...map.values()]}
