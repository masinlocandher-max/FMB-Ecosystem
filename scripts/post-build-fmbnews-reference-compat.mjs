...
for (const file of files) {
  ...
  if (!header.includes(officialLogo)) throw new Error(`FMB News official masthead logo missing after compatibility cleanup: ${file}`);
  if (html.replace(header, '').includes(officialLogo)) throw new Error(`FMB News official logo must remain masthead-only: ${file}`);
...
console.log(`Kept the official FMB News logo in the masthead-only compatibility lockup across ${files.length} generated page(s); cleaned ${updated} footer duplicate(s).`);
