  ...
  try {
    for (const [name, width, height] of viewports) {
      ...
      const state = await page.evaluate(() => {
        ...
        const controls = [...document.querySelectorAll('.fnc-menu,.fnc-nav-close,.fnc-category,.fnc-submit')]
          .filter(visible)
        ...
        const hero = document.querySelector('.fnc-brand-hero-copy');
        ...
        return {
          ...
          controls,
          localBrokenImages,
          reportCount: document.querySelectorAll('[data-fnc-card]').length + document.querySelectorAll('[data-fnc-archive-item]').length + 1,
      ...
      assert(state.bodyScrollWidth <= width, `${name}: body overflows ${state.bodyScrollWidth - width}px`);
      ...
      assert(state.reportCount === 81, `${name}: expected 81 reports, found ${state.reportCount}`);
      ...
      if (width <= 1080) {
        ...
        assert(await page.evaluate(() => document.querySelector('#fncNav')?.contains(document.activeElement)), `${name}: focus did not enter drawer`);
      ...
      const businessOnly = await page.locator('[data-fnc-card]:visible').evaluateAll((cards) => cards.every((card) => card.dataset.category === 'business'));
      assert(businessOnly, `${name}: category filter left non-business cards visible`);
      assert((await page.locator('[data-fnc-filter-status]').textContent()).includes('reports shown'), `${name}: category result status missing`);
      await page.locator('[data-fnc-filter="all"]').click();
      await page.locator('[data-fnc-search]').fill('Razon');
      assert(await page.getByText('Enrique Razon Tops Forbes Philippines 50 Richest List', { exact: true }).first().isVisible(), `${name}: search did not reveal the Razon report`);
