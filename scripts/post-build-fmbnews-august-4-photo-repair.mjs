...
const htmlFiles = await files(dist);
let changed=0;
...
const generatedHtml = (await Promise.all(htmlFiles.map(file => readFile(file,'utf8')))).join('\n');
for(const [name] of stories) {
  if(!generatedHtml.includes(local(name))) throw new Error(`Generated FMB News routes missing ${name}`);
}
