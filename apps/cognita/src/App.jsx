...
const portalEnabled = import.meta.env.VITE_ENABLE_COGNITA_PORTAL === 'true';
...
const PortalUnavailable = () => (
  <main className="apple-surface flex min-h-screen items-center px-5 py-24 sm:px-6">
    <section className="apple-card mx-auto w-full max-w-2xl p-7 text-center sm:p-10">
      <p className="apple-eyebrow">Portal maintenance</p>
      ...
      <p className="mx-auto mt-5 max-w-xl leading-7 text-slate-400">
        Cognita is completing account, permissions, enrollment, payment, and learner-record checks before portal access is reopened.
      </p>
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        <a className="apple-button-primary px-6 py-3 text-sm font-semibold" href="/#waitlist">
          Join the waitlist
        </a>
        <a className="apple-button-secondary px-6 py-3 text-sm font-medium" href="/contact">
          Contact Cognita
        </a>
      </div>
    </section>
  ...
  return (
    <Suspense fallback={<RouteLoadingFallback />}>
      <Routes>
        ...
        <Route path="/login" element={portalEnabled ? <Login /> : <PortalUnavailable />} />
        <Route path="/register" element={portalEnabled ? <Register /> : <PortalUnavailable />} />
        <Route path="/forgot-password" element={portalEnabled ? <ForgotPassword /> : <PortalUnavailable />} />
        <Route path="/reset-password" element={portalEnabled ? <ResetPassword /> : <PortalUnavailable />} />
