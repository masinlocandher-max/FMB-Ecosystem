(function(){
  ...
  function readingTime(item){
    const words=`${item.summary||''} ${item.filipino_impact||''} ${item.household_impact||''} ${item.fmb_perspective||''}`.trim().split(/\s+/).filter(Boolean).length;
  ...
  function empty(message){
    ...
    if(status)status.textContent='Filipino-first desk ready';
  }
  function render(items){
    if(!items.length){empty('Approved sources can publish automatically after they pass the Filipino-impact and safety rules.');return}
      ...
      return `<article class="news-auto-card">
        <a href="${href}">
          ...
          <div class="news-auto-card-copy">
            ...
            <p class="news-auto-summary">${escapeHtml(item.summary||item.source_excerpt||'Open the brief for the verified source and publication details.')}</p>
            <div class="news-auto-impact"><strong>What it means for Filipinos</strong><p>${escapeHtml(item.filipino_impact||'Open the brief for the Filipino impact analysis.')}</p></div>
            <div class="news-auto-source"><span>${escapeHtml(item.source_name)}</span><b>${readingTime(item)} →</b></div>
          </div>
        </a>
      </article>`;
    }).join('');
    if(status)status.textContent=`${items.length} Filipino-centered brief${items.length===1?'':'s'}`;
  ...
  async function init(){
    ...
    try{
      ...
      const {data,error}=await client.from('news_articles')
        .select('slug,title,summary,source_excerpt,filipino_impact,household_impact,fmb_perspective,category,source_name,image_url,published_at,source_published_at')
        .eq('status','published')
