(()=>{
  ...
  const setState=(form,message,state='idle')=>{
    const status=form.querySelector('[data-fmb-newsletter-status]');
  ...
  for(const form of forms){
    form.addEventListener('submit',async(event)=>{
      ...
      setState(form,'Subscribing…','loading');
      try{
        const response=await fetch(`${url}/rest/v1/fmb_news_subscribers`,{
          method:'POST',
          headers:{
            apikey:key,
        ...
        if(response.ok){
          input.value='';
          setState(form,'You’re subscribed to the FMB News Daily Brief.','success');
          return;
        }
        if(response.status===409){
          setState(form,'This email is already subscribed.','success');
          return;
        }
        throw new Error(`Subscription failed (${response.status})`);
      }catch(error){
        console.error('FMB News newsletter signup failed',error);
        setState(form,'We couldn’t subscribe you right now. Please try again.','error');
      }finally{
