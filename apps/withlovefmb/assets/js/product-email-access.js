(function(){
  ...
  function ensureModal(){
    ...
    form.addEventListener('submit',async event=>{
      ...
      const {error}=await client.auth.signInWithOtp({email,options:{shouldCreateUser:true,emailRedirectTo:redirectUrl(),data:{product_access_source:activeMode==='music'?'fmb_music':'fmb_ebooks',product_access_method:'email_link'}}});
      ...
      rememberEmail(email);status.textContent=`Check ${email}. The link will continue ${actionLabel()} without a password.`;status.classList.add('success');
    });
    return modal;
  }
  ...
  function open(options={}){
    ...
    element.querySelector('.fmb-email-copy').textContent=isMusic?'Enter your email to receive a secure link for the complete FMB Music library. New addresses are registered automatically; returning listeners use the same email as before.':'Enter your email to receive a secure link for the remaining chapters. New addresses are registered automatically; returning readers use the same email as before.';
