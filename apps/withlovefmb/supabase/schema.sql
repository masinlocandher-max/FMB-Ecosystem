...
-- ---------------------------------------------------------------------------
-- Complete member app profile, theme and Freedom Wall identity rules
-- ---------------------------------------------------------------------------
...
alter table public.profiles add constraint profiles_app_theme_check check (app_theme in ('violet','navy','forest','slate'));
alter table public.profiles drop constraint if exists profiles_avatar_preset_check;
alter table public.profiles add constraint profiles_avatar_preset_check check (avatar_preset is null or avatar_preset in ('orange','apple','grapes','cherry','peach','lemon','blueberry','watermelon'));

create or replace function private.enforce_member_profile_identity()
returns trigger language plpgsql security invoker set search_path = '' as $$
declare v_next_change timestamptz;
begin
  if new.full_name is distinct from old.full_name or new.display_name is distinct from old.display_name then
    raise exception using errcode='P0001', message='Real name cannot be changed from profile settings';
  end if;
  if new.username is distinct from old.username then
    ...
    if old.username_changed_at is not null and statement_timestamp() < v_next_change then
      raise exception using errcode='P0001', message=format('Username can be changed again on %s',to_char(v_next_change at time zone 'UTC','FMMonth DD, YYYY'));
    end if;
...
create trigger profiles_enforce_member_identity before update on public.profiles for each row execute function private.enforce_member_profile_identity();

revoke update (full_name,display_name) on public.profiles from authenticated;
grant update (username,bio,interests,avatar_url,app_theme,avatar_preset,updated_at) on public.profiles to authenticated;
...
begin
  ...
  select p.username into v_username from public.profiles as p where p.id=new.user_id and p.status='active';
...
create trigger freedom_wall_prepare_member_post before insert on public.freedom_wall_posts for each row execute function private.prepare_freedom_wall_post();
