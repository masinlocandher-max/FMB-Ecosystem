(function(){
  ...
  function localDate(value=new Date()){return new Intl.DateTimeFormat('en-PH',{weekday:'short',month:'short',day:'numeric',year:'numeric'}).format(value)}
  ...
  function moodEmoji(value){return MOOD_EMOJI[Math.max(1,Math.min(5,Number(value)||3))]||'😟'}
  function journalDateKey(entry){return entry.date||localDateKey(entry.createdAt)}
  ...
  $$('[data-go]').forEach(control=>control.addEventListener('click',event=>{event.preventDefault();go(control.dataset.go)}));
  ...
  moodButtons.forEach(button=>button.addEventListener('click',()=>{
    selectedMood={value:Number(button.dataset.mood),label:button.dataset.label,emoji:button.dataset.emoji};
  ...
  $('#save-checkin').addEventListener('click',()=>{
    ...
    const entry={id:Date.now(),date:today,createdAt:new Date().toISOString(),mood:selectedMood.value,label:selectedMood.label,emoji:selectedMood.emoji,note:$('#checkin-note').value.trim()};
    const withoutToday=entries.filter(item=>item.date!==today);
    withoutToday.unshift(entry);
    if(!write(CHECKIN_KEY,withoutToday.slice(0,365))){toast('This browser could not save the check-in.');return}
    $('#checkin-confirmation').classList.add('visible');
    toast('Today’s check-in was saved on this device.');
  ...
  function renderToday(){
    ...
    if(!entry){card.innerHTML='<h3>No check-in yet.</h3><p>There is no right mood to choose. Start with the one closest to what you feel.</p>';return}
  ...
  $('#save-journal').addEventListener('click',()=>{
    ...
    if(!text){toast('Write something before saving.');journalText.focus();return}
    const entries=read(JOURNAL_KEY);
    entries.unshift({id:Date.now(),date:localDateKey(),createdAt:new Date().toISOString(),text});
    if(!write(JOURNAL_KEY,entries.slice(0,365))){toast('This browser could not save the entry.');return}
    journalText.value='';
    $('#journal-count').textContent='0 characters';
    renderJournal();
    toast('Journal entry saved on this device.');
  });

  function renderJournal(){
    const list=$('#journal-list');
    const entries=read(JOURNAL_KEY);
    if(!entries.length){list.innerHTML='<div class="empty card">No journal entries yet. Your first saved reflection will appear here.</div>';return}
    ...
    $$('[data-delete-entry]').forEach(button=>button.addEventListener('click',()=>{
      ...
      write(JOURNAL_KEY,next);renderJournal();toast('Entry deleted.');
    }));
  }
  ...
  function renderCalendar(){
    ...
    $('#calendar-month').textContent=new Intl.DateTimeFormat('en-PH',{month:'long',year:'numeric'}).format(calendarCursor);
    ...
    journals.forEach(entry=>{const key=journalDateKey(entry);const list=journalMap.get(key)||[];list.push(entry);journalMap.set(key,list)});
    ...
    $$('[data-history-date]').forEach(button=>button.addEventListener('click',()=>{
      ...
      if(date.getMonth()!==calendarCursor.getMonth()||date.getFullYear()!==calendarCursor.getFullYear())calendarCursor=new Date(date.getFullYear(),date.getMonth(),1);
      renderCalendar();
    }));
  ...
  function renderHistoryDetail(key,checkinMap=null,journalMap=null){
    ...
    const journals=journalMap||(()=>{const map=new Map();read(JOURNAL_KEY).forEach(entry=>{const entryKey=journalDateKey(entry);const list=map.get(entryKey)||[];list.push(entry);map.set(entryKey,list)});return map})();
    const checkin=checkins.get(key);
    const dayJournals=journals.get(key)||[];
    const detail=$('#history-detail');
    const heading=escapeHtml(localDate(dateFromKey(key)));
    if(!checkin&&!dayJournals.length){detail.innerHTML=`<h3>${heading}</h3><p class="history-empty">No check-in or journal entry was saved for this date.</p>`;return}
    let html=`<h3>${heading}</h3>`;
    if(checkin){html+=`<div class="history-block"><div class="history-label">Daily check-in</div><div class="history-checkin"><span class="mood-mark" aria-hidden="true">${checkin.emoji||moodEmoji(checkin.mood)}</span><div><strong>${escapeHtml(checkin.label)}</strong><p>${checkin.note?escapeHtml(checkin.note):'No note added.'}</p></div></div></div>`}
    if(dayJournals.length){html+=`<div class="history-block"><div class="history-label">Journal ${dayJournals.length===1?'entry':'entries'}</div>${dayJournals.map(entry=>`<div class="history-journal"><time>${escapeHtml(new Intl.DateTimeFormat('en-PH',{hour:'numeric',minute:'2-digit'}).format(new Date(entry.createdAt)))}</time><p>${escapeHtml(entry.text)}</p></div>`).join('')}</div>`}
  ...
  async function breathingPhase(label,className,seconds){
    ...
    for(let value=seconds;value>0;value--){$('#breathe-time').textContent=`${label}: ${value}`;await wait(1000)}
  }
  $('#start-breathing').addEventListener('click',async()=>{
    if(breathing)return;
    breathing=true;
    const button=$('#start-breathing');button.disabled=true;button.textContent='Breathing cycle in progress';
    await breathingPhase('Inhale','inhale',4);
    await breathingPhase('Hold','hold',2);
    await breathingPhase('Exhale','exhale',6);
    $('#breathe-orb').className='breathe-orb';$('#breathe-label').textContent='Done';$('#breathe-time').textContent='Return to your natural breathing.';
    button.disabled=false;button.textContent='Begin another cycle';breathing=false;
  });

  const initial=['home','checkin','journal','history','tools','help'].includes(location.hash.slice(1))?location.hash.slice(1):'home';
